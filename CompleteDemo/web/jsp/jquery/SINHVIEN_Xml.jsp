...
<div id="mainbody"><%@ include file = "../common/header.jsp" %>
	<div id="ja-wrapper">
		...
		<div id="ja-wrapper-inner" class="clearfix">		
			<font style="font-size: x-large;">QL Sinh Viên Xml</font>
			<br /> <br />
...
function reloadSINHVIENList()
{
  ...
  jQuery("#SINHVIENGrid").setGridParam({ url:"<%=request.getContextPath() %>/SINHVIEN_Servlet?" + params});
...
$(function() {
  ...
  jQuery("#SINHVIENGrid").jqGrid({ 
      url:'<%=request.getContextPath() %>/SINHVIEN_Servlet', 
      datatype: "xml", 
                      ...
                      click: function() {
                          ...
                          if 	(isValidForm_create())
                          {
                              $.ajax({
                                  type: "POST",
                                  url: "<%=request.getContextPath()%>/SINHVIEN_Servlet",
                                  data: {"xml":"<doc><method>insert</method><MSV>" + $("#MSV_create").val()+"</MSV><Hoten>" + $("#Hoten_create").val()+"</Hoten><GT>" + $("#GT_create").val()+"</GT><NS>" + $("#NS_create").val()+"</NS><Que>" + $("#Que_create").val()+"</Que><Lop>" + $("#Lop_create").val()+"</Lop></doc>"},
                                  error: function(){
                                      $( "#dialog_error" ).dialog({
                                          modal: true,
                                          buttons: {
                      ...
                      click: function() {
                          ...
                          if 	(isValidForm_edit())
                          {
                              $.ajax({
                                  type: "POST",
                                  url: "<%=request.getContextPath()%>/SINHVIEN_Servlet",
                                  data: {"xml":"<doc><method>update</method><MSV>" + $("#MSV_edit").val()+"</MSV><Hoten>" + $("#Hoten_edit").val()+"</Hoten><GT>" + $("#GT_edit").val()+"</GT><NS>" + $("#NS_edit").val()+"</NS><Que>" + $("#Que_edit").val()+"</Que><Lop>" + $("#Lop_edit").val()+"</Lop></doc>"},
                                  error: function(){
              ...
              $( "#dialog_confirm_delete" ).dialog({
                  ...
                  buttons: {
                      ...
                      $.ajax({
                          type: "POST",
                          url: "<%=request.getContextPath()%>/SINHVIEN_Servlet",
                          data: {"xml":"<doc><method>delete</method><MSV>" + ret.MSV + "</MSV><Hoten>" + $("#Hoten_edit").val()+"</Hoten><GT>" + $("#GT_edit").val()+"</GT><NS>" + $("#NS_edit").val()+"</NS><Que>" + $("#Que_edit").val()+"</Que><Lop>" + $("#Lop_edit").val()+"</Lop></doc>"},
                          error: function(){
                              $( "#dialog_error" ).dialog({
                                  modal: true,
                                  buttons: {
...
<script type="text/javascript">
	...
	$("#NS_edit").datepicker({ 
		...
		yearSuffix: '' 
			...
			$.ajax({ 
				type: "GET", 
				url: '<%=request.getContextPath() %>/SINHVIEN_Servlet?method=getCbbData&cbbName=Que', 
				dataType: "xml", 
				success: function(xml) { 
					...
					$(xml).find('option').each(function(){ 
						...
						Que_create.append("<option value='"+ value +"'>"+text+"</option>"); 
						Que_edit.append("<option value='"+ value +"'>"+text+"</option>"); 
					}); 
				} 
			}); 
